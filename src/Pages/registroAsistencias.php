...
$action = $_GET['action'] ?? 'consulta_asistencia';
    ...
    <div class="bg-white shadow-md rounded-lg">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td>
                                    <a href="?action=edit&id=<?= $student['Id_Asistencia'] ?>"
        ...
        <?php elseif ($action === 'edit'):
            ...
            $query = "SELECT t1.*, t2.*, t3.* 
                      ...
                      INNER JOIN estudiantes as t2 ON t1.Estudiantes_Id_Estudiantes = t2.Id_Estudiantes 
                      INNER JOIN cursos as t3 ON t1.Cursos_Id_Cursos = t3.Id_Cursos
                      WHERE t1.Id_Asistencia = $id";
            $result = mysqli_query($conn, $query);
            $student = mysqli_fetch_assoc($result);
        ?>
            <?php if (!$student): ?>
                <p class="text-red-500">Estudiante no encontrado.</p>
                <a href="?redireccionar=consulta_asistencia" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                    Volver
                </a>
            <?php else: ?>
                <div class="max-w-md mx-auto bg-white shadow-md rounded-lg p-6">
                    <h2 class="text-2xl font-bold mb-6 text-center text-blue-600">Editar Asistencia</h2>
                    <form method="POST" action="../../src/Asistencia/saveAsis.php" class="space-y-4">
                        <input type="hidden" name="id" value="<?= $id ?>">
                        <div>
                            <p class="block text-gray-700 font-bold mb-2">Curso </p>
                            <p class="block text-gray-700 font-bold mb-2"><?= $student['Nombre_Cursos'] ?> </p>
                        </div>
                        <div>
                            <p class="block text-gray-700 font-bold mb-2">Nombre </p>
                            <p class="block text-gray-700 font-bold mb-2"><?= $student['Nombre_Estudiantes'] ?> <?= $student['Apellido_Estudiantes'] ?></p>
                        </div>

                        <div>
                            <label class="block text-gray-700 font-bold mb-2">Fecha de Registro</label>
                            <input type="date" name="Fecha_Asistencia" required value="<?= $student['Fecha_Asistencia'] ?>"
                                class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-gray-700 font-bold mb-2">Presente</label>
                            <input type="checkbox" name="Presente" value="Asistencia"
                                <?= $student['Presente'] == 1 ? 'checked' : '' ?>
                                class="w-full h-6 px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div class="flex justify-between">
                            <a href="?redireccionar=consulta_asistencia" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                                Cancelar
                            </a>
                            <button type="submit" name="editAsistencia" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                                Actualizar
                            </button>
                        </div>
                    </form>
